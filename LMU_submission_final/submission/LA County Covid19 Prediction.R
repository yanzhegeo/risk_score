...
# load LA Country covid19 case data from tableau and JHU
# https://www.tableau.com/covid-19-coronavirus-data-resources
# Note that the total populaiton of LA county is 10039107

# the LA County.csv is attached with our submission
...
#        the stages can be lock-down and gradual reopening 
#
# lockdown fatigue: 
...
# total obs in our data is 73, 75% for training gives us 55 obs
train_len <- round(nrow(LA) * 0.75)
# we just split 10 from the remaining 18 obs as validate dataset
# leave 8 obs for test
# note that this setting is consistent with 
# many COVID19 predictive models that test predictions for one week 
valid_len <- 10
...
Loss_Total <- function (par_id) {
    ...
    loss_valid1 <- mean(error1[(train_len + 1): (train_len + valid_len + 1)])
    loss_test1 <- mean(error1[(train_len + valid_len + 2): nrow(LA)])   
  
    # MSE of (deaths + cases)
    error2 <- (simu$Dead - LA$death_total)^2 + (N - simu$S - LA$case_total)^2
    loss_train2 <- mean(error2[1: train_len])
    loss_valid2 <- mean(error2[(train_len + 1): (train_len + valid_len + 1)])
    loss_test2 <- mean(error2[(train_len + valid_len + 2): nrow(LA)])   

    # MSE of daths + ABS(cases)
    error3 <- (simu$Dead - LA$death_total)^2 + abs(N - simu$S - LA$case_total)
    loss_train3 <- mean(error3[1: train_len])
    loss_valid3 <- mean(error3[(train_len + 1): (train_len + valid_len + 1)])
    loss_test3 <- mean(error3[(train_len + valid_len + 2): nrow(LA)])   
  
    # MSE of case
    error4 <- (N - simu$S - LA$case_total)^2
    loss_train4 <- mean(error4[1: train_len])
    loss_valid4 <- mean(error4[(train_len + 1): (train_len + valid_len + 1)])
    loss_test4 <- mean(error4[(train_len + valid_len + 2): nrow(LA)])   
...
#       form those models, we choice 10 with the smallest loss in validate dataset
...
prediction <- prediction %>% 
  mutate(week_id = c(rep(c(1:10), each = 7), c(11, 11, 11))) %>% group_by(week_id) %>% 
...
prediction$weight <- rep(par_group$weight, each = 80)
